...
# def test_ecommerce_scraper(playwright_local_grid_page):
#     scraper = EcommerceScraper(playwright_local_grid_page)
#     scraper.select_product_category()
#     title = scraper.page_title()
#     print(title)

#     assert title == "Laptops & Notebooks", "Expected page title to be 'Laptops & Notebooks'"
...
def test_product_title_list(page, set_test_status):
    ...
    assert isinstance(product_title_list, list), f"Expected {product_title_list} to be a list"
    assert len(product_title_list) == 15, f"Expected {product_title_list} to contain product names"
...
def test_product_price_list(page, set_test_status):
    ...
    assert isinstance(product_price_list, list), f"Expected {product_price_list} to be a list"
    assert len(product_price_list) == 15, f"Expected {product_price_list} to contain product prices"	
...
def test_product_image_link_list(page, set_test_status):
    ...
    assert len(image_link_list) == 15, f"Expected {image_link_list} to contain product-image links"
...
def test_product_detail_list(page, set_test_status):
    ...
    assert isinstance(product_detail_list, list), f"Expected {product_detail_list} to be a list"
    assert isinstance(product_detail_list[0], dict), f"Expected {product_detail_list} to be Python dict"
    assert len(product_detail_list) == 15, f"Expected {product_detail_list} to contain product detail dictionary"	
